...
export type ApiErrorKind = 'canceled' | 'timeout' | 'network' | 'response' | 'invalid-response' | 'unknown'
...
function createResponseApiError(error: AxiosError): ApiError {
    ...
    return {
        kind: 'response',
        message: responseMessage ?? error.message ?? buildUnexpectedResponseMessage(status),
        status,
        url,
        code: error.code ?? null,
